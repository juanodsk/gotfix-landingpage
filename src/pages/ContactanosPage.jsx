import Reseñas from "../components/Reseñas.jsx";
import Navbar from "../components/Navbar.jsx";
import Footer from "../components/Footer.jsx";
import { FaWhatsapp } from "react-icons/fa";
import Local from "../assets/localafuera.JPG";
import SEO from "../components/SEO.jsx";
import { gotfixBusinessSchema } from "../data/seo.js";

const contactDescription =
  "Contacta a GotFix en Neiva. Estamos en Calle 21 #5bis - 21 edificio Las Ceibas y atendemos reparaciones Apple por WhatsApp al (+57) 312 504 2689.";

const contactSchema = [
  gotfixBusinessSchema,
  {
    "@context": "https://schema.org",
    "@type": "ContactPage",
    "@id": "https://gotfix.co/contactanos#contacto",
    url: "https://gotfix.co/contactanos",
    name: "Contáctanos | GotFix Neiva",
    description: contactDescription,
    about: {
      "@id": "https://gotfix.co/#gotfix",
    },
  },
];

const ContactanosPage = () => {
  return (
    <>
      <SEO
        title="Contáctanos | GotFix Neiva"
        description={contactDescription}
        path="/contactanos"
        schema={contactSchema}
      />
      <Navbar />
      <section className="py-10 bg-white">
        <div className="container mx-auto px-6">
          <div className="mt-10">
            <h1 className="text-3xl font-bold text-center mb-4 text-gray-900">
              Contáctanos en GotFix Neiva
            </h1>
            <p className="mx-auto mb-8 max-w-3xl text-center text-lg text-gray-700">
              Servicio técnico Apple en Neiva para diagnóstico, mantenimiento y
              reparación de iPhone, MacBook, iPad e iMac. Escríbenos por
              WhatsApp o visítanos en nuestro punto físico.
            </p>
            <h2 className="text-2xl font-semibold text-center mb-4">
              ¿Dónde estamos ubicados?
            </h2>
            <div className="flex justify-center">
              <iframe
                src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d512.6669721455606!2d-75.29196736983681!3d2.9368210463434754!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x8e3b759b29731d61%3A0x2eef2bcfde0bc115!2zR09URklYwq4!5e0!3m2!1ses-419!2sco!4v1751588792051!5m2!1ses-419!2sco"
                width="100%"
                height="400"
                allowFullScreen=""
                loading="lazy"
                referrerPolicy="no-referrer-when-downgrade"
                className="rounded-lg shadow-lg w-full md:w-[80%]"
              ></iframe>
            </div>
            <address className="text-xl not-italic font-bold text-center mb-6 mt-5 text-gray-900">
              Calle 21 #5bis - 21 edificio Las Ceibas, Neiva, Huila, Colombia
            </address>
            <div className="flex justify-center">
              <img
                src={Local}
                alt="Local GotFix"
                className="rounded-xl max-w-100 md:max-w-150 h-auto shadow-xl"
              />
            </div>
          </div>
          <h2 className="text-3xl font-bold text-center mb-6 mt-8">
            Agenda tu reparación Apple
          </h2>
          <p className="text-gray-700 text-lg mb-4 text-center">
            Atención de lunes a sábado por WhatsApp en el (+57) 312 504 2689.
            Te orientamos sobre el diagnóstico, el repuesto adecuado y la
            garantía GotFix de 6 meses.
          </p>
          <div className="flex justify-center">
            <a
              href="https://wa.link/7jzopx"
              target="_blank"
              rel="noopener noreferrer"
              aria-label="WhatsApp"
              className="bg-[#25d366] hover:bg-[#25d365b2] text-white cursor-pointer transition delay-10 px-6 py-3 rounded font-semibold  z-50 inline-flex items-center"
            >
              <FaWhatsapp className="mb-[2px] mr-2 text-[22px]" />
              Cotiza tu reparación
            </a>
          </div>
        </div>
        <div className="mt-10">{/* ...iframe como te lo dejé arriba... */}</div>
      </section>
      <Reseñas />
      <Footer />
    </>
  );
};

export default ContactanosPage;
